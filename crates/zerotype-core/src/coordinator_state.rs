...
pub fn request_stop_during_starting_state(state: &mut SessionState) -> bool {
...
pub fn finish_starting_session_state(
    state: &mut SessionState,
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum StartupRaceStatus {
    ActiveStarting,
    CancelRaced,
    StaleContinuation,
}

pub fn startup_race_status(
    state: &SessionState,
...
pub fn begin_cancel_session_state(state: &mut SessionState) -> Option<CancelDecision> {
...
pub fn finish_cancel_session_state(state: &mut SessionState, decision: CancelDecision) {
...
pub fn finish_cancelled_processing_state(
    state: &mut SessionState,
    session_id: SessionId,
) -> bool {
...
pub fn start_processing_if_listening(state: &mut SessionState) -> Option<SessionId> {
...
pub fn begin_recording_abort_before_restore(
    state: &mut SessionState,
...
pub fn publish_abort_idle_after_restore(state: &mut SessionState, session_id: SessionId) {
    if state.session_id == session_id {
        state.phase = SessionPhase::Idle;
    }
}
