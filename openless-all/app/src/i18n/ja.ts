...
export const ja: typeof zhCN = {
  ...
  onboarding: {
    ...
    actionRestart: 'アクセシビリティをリセットして ZeroType を再起動',
    actionGrant: '許可する',
    actionRequestMic: '許可ダイアログを表示',
    accessibilityHint: '許可後は **ZeroType を完全に終了** してから再起動してください（macOS TCC の仕様）。',
    footerHint: 'すべての権限が揃うとこのガイドは自動で閉じます。閉じない場合はメニューバーの ZeroType → 終了 から再起動してください。',
    androidContinue: 'アプリに進む',
    androidFooterHint: '音声入力にはマイク権限が必要です。上の「許可ダイアログを表示」をタップするか、先にアプリへ進み、概要ページで後から許可してください。',
    androidTitle: 'ZeroType を設定',
    ...
    androidSteps: {
      ...
      microphoneDesc: 'Android のシステム権限カードを表示し、ZeroType の録音を許可します。',
  ...
  settings: {
    ...
    recording: {
      ...
      windowsShowOpenlessInKeyboardListLabel: 'キーボード一覧に ZeroType を表示',
      windowsShowOpenlessInKeyboardListDesc: 'オフにすると Win+Space で ZeroType に切り替わりません。SendInput 挿入には影響しません。オンに戻すと一覧に再表示されます。',
      windowsShowOpenlessInKeyboardListError: 'キーボード一覧を更新できません：ZeroType TSF IME が未インストールか、システムが変更を拒否しました。',
      ...
      startupAtBootDesc: 'ログイン時に ZeroType を自動起動。',
    ...
    providers: {
      ...
      codexOAuthNotice: 'Codex OAuth はローカルの Codex ログイン状態（~/.codex/auth.json）を使用します。ZeroType は API Key や Base URL を保存しません。',
      ...
      bailianModelRecordedFileHint: '録音ファイルモデル · 話し終えてから一括で文字起こし（1 本 ≤ 5 分）。',
    ...
    permissions: {
      title: '権限',
      descAcc: 'ZeroType は正常動作のため以下のシステム権限が必要です。許可後は通常、App を完全に終了して再起動する必要があります。',
      descNoAcc: 'ZeroType はマイクへのアクセスと、グローバルショートカット監視状態を通じてネイティブフックの正常動作を判定する必要があります。',
      micLabel: 'マイク',
      micDesc: '音声入力の取得に使用します。',
      accLabel: 'アクセシビリティ',
      ...
      windowsImeDesc: '音声セッション中に ZeroType TSF IME へ一時的に切り替え、クリップボード入力の制限を回避します。',
      ...
      windowsIme: {
        installed: 'インストール済み。音声入力時に ZeroType IME へ一時的に切り替えます。',
        notInstalled: '未インストール。ZeroType は現在クリップボード / WM_PASTE フォールバックを使用しています。',
        registrationBroken: '登録が破損しています。ZeroType IME を再インストールしてください。',
    ...
    about: {
      ...
      updateDialog: {
        available: {
          title: '新しいバージョンがあります',
          desc: 'ZeroType {{version}} が見つかりました。今すぐ更新しますか？',
        },
        downloading: {
          title: 'アップデートをダウンロード中',
          desc: 'ZeroType {{version}} をダウンロード中です。アプリを開いたままにしてください。',
        },
        downloaded: {
          title: 'アップデートの準備完了',
          desc: 'ZeroType {{version}} のインストールが完了しました。今すぐ自動再起動して適用しますか？',
        },
        installing: {
          title: 'アップデートをインストール中',
          desc: 'ZeroType {{version}} をインストール中です。アプリを開いたままにしてください。',
        },
        install: '今すぐ更新',
        androidInstall: 'ダウンロードしてインストーラを開く',
        androidInstalled: {
          title: 'システムインストーラを開きました',
          desc: '画面の指示に従ってインストールしてください。完了後 ZeroType を再度開くと {{version}} が使えます。',
        },
  ...
  localAsr: {
    ...
    storageDesc: 'カスタム保存先では選択フォルダ配下に ZeroType/models を作成し、既存モデルを移行します。移行前にダウンロードをキャンセルし、読み込み済みモデルを解放します。',
    storageChooseTitle: 'ローカルモデル保存先の親フォルダを選択',
    storageChangeConfirm: '既存のローカルモデルを {{path}}/ZeroType/models に移動します。先にダウンロードをキャンセルし、読み込み済みモデルを解放します。続行しますか？',
