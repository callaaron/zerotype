...
export const zhCN = {
  ...
  onboarding: {
    ...
    actionRestart: '重置授权并重启 ZeroType',
    actionGrant: '授权',
    actionRequestMic: '弹出授权',
    accessibilityHint: '授权后必须**完全退出 ZeroType** 再重新打开（macOS TCC 规则）。',
    footerHint: '授权全部完成后此引导自动关闭。如果一直不消失，从菜单栏 ZeroType → 退出，重新打开 App。',
    ...
    androidSteps: {
      ...
      microphoneDesc: '调用 Android 系统授权卡片，允许 ZeroType 录制语音。',
      accessibilityTitle: '无障碍服务',
      accessibilityDesc: '用于把识别结果粘贴回当前输入框，并辅助检测输入环境。',
      overlayPermissionTitle: '悬浮窗权限',
      overlayPermissionDesc: '允许 ZeroType 在其他应用上显示录音控制按钮。',
  ...
  settings: {
    ...
    recording: {
      ...
      windowsShowOpenlessInKeyboardListLabel: '在键盘列表中显示 ZeroType',
      windowsShowOpenlessInKeyboardListDesc: '关闭后 Win+Space 切换输入法时不会出现 ZeroType；SendInput 插入不受影响。重新开启本项可恢复显示。',
      windowsShowOpenlessInKeyboardListError: '无法更新键盘列表：ZeroType TSF 输入法未安装或系统拒绝操作。',
      ...
      startupAtBoot: '开机自启',
      startupAtBootDesc: '登录系统时自动启动 ZeroType。',
    ...
    providers: {
      ...
      codexOAuthNotice: 'Codex OAuth 使用本机 Codex 登录状态（~/.codex/auth.json），无需在 ZeroType 中保存 API Key 或 Base URL。',
      ...
      bailianModelRecordedFileHint: '录音文件模型 · 说完后整段转写（单条 ≤ 5 分钟）。',
    ...
    permissions: {
      title: '权限',
      descAcc: 'ZeroType 需要以下系统权限。授权后通常要完全退出 App 重启一次才生效。',
      ...
      windowsImeDesc: '语音输入时临时切到 ZeroType TSF，绕过剪贴板限制。',
      ...
      windowsIme: {
        installed: '已安装，按需切到 ZeroType 输入法。',
        notInstalled: '未安装，走剪贴板 / WM_PASTE 兜底。',
        registrationBroken: '注册损坏，请重装 ZeroType 输入法。',
    ...
    about: {
      ...
      updateDialog: {
        available: {
          title: '发现新版本',
          desc: '发现 ZeroType {{version}}，是否现在更新？',
        },
        downloading: {
          title: '正在下载更新',
          desc: '正在下载 ZeroType {{version}}，请保持应用打开。',
        },
        downloaded: {
          title: '更新已准备好',
          desc: 'ZeroType {{version}} 已安装完成。是否现在自动重启以应用更新？',
        },
        installing: {
          title: '正在安装更新',
          desc: '正在安装 ZeroType {{version}}，请保持应用打开。',
        },
        install: '现在更新',
        androidInstall: '下载并打开安装器',
        androidInstalled: {
          title: '系统安装器已打开',
          desc: '请按系统提示完成安装。安装后重新打开 ZeroType 即可使用 {{version}}。',
        },
  ...
  localAsr: {
    ...
    storageDesc: '自定义目录会在所选位置下创建 ZeroType/models，并自动迁移现有模型；迁移前会取消下载和释放已加载模型。',
    storageChooseTitle: '选择本地模型存储父目录',
    storageChangeConfirm: '将把现有本地模型迁移到 {{path}}/ZeroType/models。迁移前会自动取消下载并释放已加载模型。是否继续？',
