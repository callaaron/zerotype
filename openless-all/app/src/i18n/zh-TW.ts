...
export const zhTW: typeof zhCN = {
  ...
  onboarding: {
    ...
    actionRestart: '重置授權並重新啟動 ZeroType',
    actionGrant: '授權',
    actionRequestMic: '彈出授權',
    accessibilityHint: '授權後必須**完全退出 ZeroType** 再重新打開（macOS TCC 規則）。',
    footerHint: '授權全部完成後此引導自動關閉。如果一直不消失，從菜單欄 ZeroType → 退出，重新打開 App。',
    androidContinue: '先進入應用',
    androidFooterHint: '聽寫需要麥克風權限。可點擊上方「彈出授權」，或先進入應用後在概覽頁繼續授權。',
    androidTitle: '配置 ZeroType',
    ...
    androidSteps: {
      ...
      microphoneDesc: '調用 Android 系統授權卡片，允許 ZeroType 錄製語音。',
      accessibilityTitle: '無障礙服務',
      accessibilityDesc: '用於把識別結果貼回當前輸入框，並輔助檢測輸入環境。',
      overlayPermissionTitle: '懸浮窗權限',
      overlayPermissionDesc: '允許 ZeroType 在其他應用上顯示錄音控制按鈕。',
  ...
  settings: {
    ...
    recording: {
      ...
      windowsShowOpenlessInKeyboardListLabel: '在鍵盤列表中顯示 ZeroType',
      windowsShowOpenlessInKeyboardListDesc: '關閉後 Win+Space 切換輸入法時不會出現 ZeroType；SendInput 插入不受影響。重新開啟本項可恢復顯示。',
      windowsShowOpenlessInKeyboardListError: '無法更新鍵盤列表：ZeroType TSF 輸入法未安裝或系統拒絕操作。',
      ...
      startupAtBoot: '開機自啓',
      startupAtBootDesc: '登錄系統時自動啓動 ZeroType。',
    ...
    providers: {
      ...
      codexOAuthNotice: 'Codex OAuth 使用本機 Codex 登入狀態（~/.codex/auth.json），無需在 ZeroType 中保存 API Key 或 Base URL。',
      ...
      bailianModelRecordedFileHint: '錄音檔模型 · 說完後整段轉寫（單條 ≤ 5 分鐘）。',
    ...
    permissions: {
      title: '權限',
      descAcc: 'ZeroType 需要以下系統權限才能正常工作。授權後通常需要完全退出 App 重啓一次才生效。',
      descNoAcc: 'ZeroType 需要麥克風可用，並依賴全局快捷鍵監聽狀態判斷 native hook 是否正常工作。',
      micLabel: '麥克風',
      micDesc: '用於捕獲你的語音輸入。',
      accLabel: '輔助功能',
      ...
      windowsImeDesc: '用於在語音會話期間臨時切換到 ZeroType TSF 輸入法，避免剪貼板插入限制。',
      ...
      windowsIme: {
        installed: '已安裝。語音輸入時會臨時切換到 ZeroType 輸入法。',
        notInstalled: '未安裝。ZeroType 正在使用剪貼板 / WM_PASTE 兜底。',
        registrationBroken: '註冊已損壞。請重新安裝 ZeroType 輸入法。',
    ...
    about: {
      ...
      updateDialog: {
        available: {
          title: '發現新版本',
          desc: '發現 ZeroType {{version}}，是否現在更新？',
        },
        downloading: {
          title: '正在下載更新',
          desc: '正在下載 ZeroType {{version}}，請保持應用打開。',
        },
        downloaded: {
          title: '更新已準備好',
          desc: 'ZeroType {{version}} 已安裝完成。是否現在自動重啓以應用更新？',
        },
        installing: {
          title: '正在安裝更新',
          desc: '正在安裝 ZeroType {{version}}，請保持應用打開。',
        },
        install: '現在更新',
        androidInstall: '下載並開啟安裝器',
        androidInstalled: {
          title: '系統安裝器已開啟',
          desc: '請依系統提示完成安裝。安裝後重新開啟 ZeroType 即可使用 {{version}}。',
        },
  ...
  localAsr: {
    ...
    storageDesc: '自訂目錄會在所選位置下建立 ZeroType/models，並自動遷移現有模型；遷移前會取消下載和釋放已載入模型。',
    storageChooseTitle: '選擇本地模型儲存父目錄',
    storageChangeConfirm: '將把現有本地模型遷移到 {{path}}/ZeroType/models。遷移前會自動取消下載並釋放已載入模型。是否繼續？',
