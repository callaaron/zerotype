...
export const ko: typeof zhCN = {
  ...
  onboarding: {
    welcome: 'ZeroType 에 오신 것을 환영합니다',
    ...
    actionRestart: '접근성 권한 재설정 후 ZeroType 재시작',
    actionGrant: '허용',
    actionRequestMic: '권한 대화상자 표시',
    accessibilityHint: '허용 후에는 **ZeroType 를 완전히 종료** 한 다음 다시 실행해야 합니다(macOS TCC 규칙).',
    footerHint: '모든 권한이 부여되면 이 가이드는 자동으로 닫힙니다. 닫히지 않으면 메뉴 막대의 ZeroType → 종료 후 앱을 다시 실행해 주세요.',
    androidContinue: '앱으로 계속',
    androidFooterHint: '받아쓰기에는 마이크 권한이 필요합니다. 위의 권한 요청을 탭하거나, 앱으로 먼저 들어가 개요 페이지에서 나중에 허용할 수 있습니다.',
    androidTitle: 'ZeroType 설정',
    ...
    androidSteps: {
      ...
      microphoneDesc: 'Android 시스템 권한 카드를 표시하고 ZeroType 녹음을 허용합니다.',
  ...
  style: {
    ...
    pack: {
      ...
      promptChars: '{{count}}자',
      runtimeTitle: 'ZeroType 런타임 추가 지시',
  ...
  settings: {
    ...
    recording: {
      ...
      windowsShowOpenlessInKeyboardListLabel: '키보드 목록에 ZeroType 표시',
      windowsShowOpenlessInKeyboardListDesc: '끄면 Win+Space로 ZeroType에 전환되지 않습니다. SendInput 삽입에는 영향 없습니다. 다시 켜면 목록에 복원됩니다.',
      windowsShowOpenlessInKeyboardListError: '키보드 목록을 업데이트할 수 없습니다: ZeroType TSF IME가 설치되지 않았거나 시스템이 변경을 거부했습니다.',
      ...
      startupAtBootDesc: '로그인 시 ZeroType 자동 시작.',
    ...
    providers: {
      ...
      codexOAuthNotice: 'Codex OAuth는 로컬 Codex 로그인 상태(~/.codex/auth.json)를 사용합니다. ZeroType는 API Key나 Base URL을 저장하지 않습니다.',
      ...
      bailianModelRecordedFileHint: '녹음 파일 모델 · 말을 마친 뒤 전체 전사(한 클립 ≤ 5분).',
    ...
    permissions: {
      title: '권한',
      descAcc: 'ZeroType 가 정상 작동하려면 다음 시스템 권한이 필요합니다. 허용 후에는 일반적으로 앱을 완전히 종료한 후 재시작해야 적용됩니다.',
      descNoAcc: 'ZeroType 는 마이크 사용과 전역 단축키 감지 상태를 통해 네이티브 후크의 정상 동작을 판정해야 합니다.',
      micLabel: '마이크',
      micDesc: '음성 입력을 캡처하기 위해 사용합니다.',
      accLabel: '접근성',
      ...
      windowsImeDesc: '음성 세션 동안 ZeroType TSF 입력기로 일시적으로 전환하여 클립보드 입력 제한을 회피하기 위해 사용.',
      ...
      windowsIme: {
        installed: '설치됨. 음성 입력 시 ZeroType 입력기로 일시 전환됩니다.',
        notInstalled: '설치되지 않음. ZeroType 는 현재 클립보드 / WM_PASTE 폴백을 사용합니다.',
        registrationBroken: '등록이 손상되었습니다. ZeroType 입력기를 재설치하세요.',
    ...
    about: {
      ...
      updateDialog: {
        available: {
          title: '새 버전 발견',
          desc: 'ZeroType {{version}} 을(를) 발견했습니다. 지금 업데이트하시겠습니까?',
        },
        downloading: {
          title: '업데이트 다운로드 중',
          desc: 'ZeroType {{version}} 을(를) 다운로드 중입니다. 앱을 열어 두세요.',
        },
        downloaded: {
          title: '업데이트 준비 완료',
          desc: 'ZeroType {{version}} 설치가 완료되었습니다. 지금 자동 재시작하여 적용하시겠습니까?',
        },
        installing: {
          title: '업데이트 설치 중',
          desc: 'ZeroType {{version}} 을(를) 설치 중입니다. 앱을 열어 두세요.',
        },
        install: '지금 업데이트',
        androidInstall: '다운로드 후 설치 프로그램 열기',
        androidInstalled: {
          title: '시스템 설치 프로그램이 열렸습니다',
          desc: '안내에 따라 설치를 완료하세요. 설치 후 ZeroType를 다시 열면 {{version}}을 사용할 수 있습니다.',
        },
  ...
  localAsr: {
    ...
    storageDesc: '사용자 지정 저장소는 선택한 폴더 아래에 ZeroType/models 를 만들고 기존 모델을 이동합니다. 이동 전에 다운로드를 취소하고 로드된 모델을 해제합니다.',
    storageChooseTitle: '로컬 모델 저장 상위 폴더 선택',
    storageChangeConfirm: '기존 로컬 모델을 {{path}}/ZeroType/models 로 이동합니다. 먼저 다운로드를 취소하고 로드된 모델을 해제합니다. 계속할까요?',
